#!/usr/bin/env bash
# Lightweight RAM/trading focus monitor — no SSD writes, read-only checks.
...
while true; do
  ...
  echo "[$ts] focus=$status | HANOON=${hanoon_pid:-—} Halim=${halim_pid:-—} | free=${ram_free_mb:-?}MB active=${ram_active_mb:-?}MB"
