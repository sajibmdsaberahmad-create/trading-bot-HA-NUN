...
class IBConnector:
    ...
    def _on_error(self, reqId, errorCode, errorString, contract):
        ...
        # IB tick-by-tick cap (10190) or unsupported Last ticks (10189) → 5s bars
        if errorCode in (10189, 10190):
            try:
                ...
                if errorCode == 10189:
                    log.info(
                        f"  📡 {ticker or '?'}: tick-by-tick not supported — using 5s bars"
                    )
                else:
                    log.info(
                        f"  📡 {ticker or '?'}: tick-by-tick cap — using 5s bars"
                    )
            except Exception:
                log.info(f"IB tick-by-tick unavailable ({errorCode}) — using 5s bars")
            return
